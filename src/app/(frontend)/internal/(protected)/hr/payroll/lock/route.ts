...
export async function POST(request: Request) {
  ...
  try {
    ...
    await lockPayrollRun({
      req: { payload, user } as any,
      runID: Number(runId),
    })

    const redirectURL = buildRedirectURL(request)
    redirectURL.searchParams.set('success', 'Payroll run locked successfully.')
    return NextResponse.redirect(redirectURL, 303)
  } catch (error) {
    const redirectURL = buildRedirectURL(request)
    redirectURL.searchParams.set('error', error instanceof Error ? error.message : 'Unable to lock payroll run.')
    return NextResponse.redirect(redirectURL, 303)
  }
}
